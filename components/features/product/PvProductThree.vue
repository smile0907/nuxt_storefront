<template>
	<div class="product-default inner-quickview inner-icon">
		...
		<div class="product-details">
			...
			<div
				class="product-nav-filter"
				:class="vColors[0].image.length > 0 ? 'product-nav-thumbs' : 'product-nav-dots'"
				v-if="vColors.length > 0"
			>
				<a
					href="javascript:;"
					v-for="(item, index) in vColors.slice(0, 3)"
					:key="'product-variant-' + index"
					:style="{backgroundColor: item.text}"
					@click="toggleColorItem(item)"
					:class="{active: curColor === item}"
				>
					<img
						...
						alt="thumb-picture"
						v-if="item.image.length > 0 && item.image"
					/>
				</a>
			</div>
		</div>
...
export default {
	...
	mounted: function() {
		...
		if (
			...
			this.product.variants[0].colors[0]
		) {
			this.product.variants.forEach(item => {
				if (
